                                         ...
                                         WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
        ...
        case WM_PAINT:  
			 ...
			 DoUpdate();
			 /*PAINTSTRUCT ps1;
			 HDC hdc = BeginPaint(graphicsWindow, &ps1);
			 HDC hdcmem = CreateCompatibleDC(hdc);
			 HANDLE hbitmap = LoadImage(NULL, "C:\Users\HUAWEI\Desktop\shezhi1.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			 BITMAP bmap;
			 GetObject(hbitmap, sizeof(BITMAP), &bmap);
			 SelectObject(hdcmem, hbitmap);
			 BitBlt(hdc, 16, 12, bmap.bmWidth, bmap.bmHeight, hdcmem, 0, 0, SRCCOPY);
			 DeleteDC(hdcmem);
			 DeleteObject(hbitmap);
			 EndPaint(graphicsWindow, &ps1);*/
			 ...
			 hbmp = (HBITMAP)LoadImage(NULL, "‪C:\Users\HUAWEI\Desktop\shezhi1.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			 GetObject(hbmp, sizeof(BITMAP), &bmp);	//得到一个位图对象

			 SelectObject(hdcMem, hbmp);
			 BitBlt(hdc, 0, 0, pixelWidth, pixelHeight, hdcMem, 0, 0, SRCCOPY);		//显示位图
